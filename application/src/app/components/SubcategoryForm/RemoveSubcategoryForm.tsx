...
export const RemoveSubcategoryForm: FC<{}> = () => {
  const { mutate: mutateSWR } = useSWRConfig();
  const router = useRouter();
  const removeSubcategoryId = useRouteParam("removeSubcategory");
  const { mutate, loading } = useMutate(
    `${API_SUBCATEGORY_URL}/${removeSubcategoryId}`,
    "DELETE"
  );
  ...
  return (
    <Dialog open={true} onBackdropClick={closeModal}>
      <DialogTitle>Remove subcategory</DialogTitle>
      <DialogContent>
        <DialogContentText>
...
export default RemoveSubcategoryForm;
